...
# a directory containing all nagios configs:
$nagios_cfgdir = "/var/local/puppet/nagios"
...
class nagios2 {
	...
	replace {
		...
		cap_service_checks:
			...
			replacement => "max_concurrent_checks=20",
			notify => Service[nagios2];
	}

	append_if_no_such_line { include_cfgdir:
		file => "/etc/nagios2/nagios.cfg",
