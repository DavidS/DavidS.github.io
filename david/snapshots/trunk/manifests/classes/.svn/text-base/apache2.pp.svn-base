...
class apache2 {
	...
	case $apache2_ssl {
		enabled: { 
			apache2_module { "ssl": ensure => present }
...
define apache2_site ( $ensure = 'present', $require = 'apache2' ) {
	case $ensure {
		'present' : {
...
define apache2_module ( $ensure = 'present', $require = 'apache2' ) {
	case $ensure {
		'present' : {
